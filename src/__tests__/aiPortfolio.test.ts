...
function price(symbol: string, priceUsd: number, source: 'avnu' | 'static' = 'static'): AssetPrice {
  return { priceUsd, source, priceFetchedAt: NOW };
}

const PRICES: Record<string, AssetPrice> = {
  [STRK]: price('STRK', 0.4, 'avnu'),
  [ETH]: price('ETH', 2700, 'avnu'),
  [USDC]: price('USDC', 1),
};

describe('buildPortfolioSummary', () => {
  it('computes USD values, allocation percentages, liquidity and price freshness', () => {
    ...
    expect(strk?.priceSource).toBe('avnu');
    expect(strk?.priceFetchedAt).toBe(NOW);
  });
