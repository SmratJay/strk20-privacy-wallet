...
import type { AccountAdapter, AccountDeployment } from "./types";
...
/**
 * True when `address` has an on-chain class hash (i.e. the account contract is deployed).
 * False when the RPC reports the contract as not found / undeployed. Fails closed on any
 * other RPC error (treats it as not deployed so the caller can retry safely).
 */
...
/**
 * Ready account adapter bound to a public key. Implements the `AccountAdapter` seam so the
 * wallet core can deploy and probe accounts without knowing Ready specifics.
 */
export class ReadyAccountAdapter implements AccountAdapter {
  ...
  constructor(publicKey: string, classHash: string = READY_SEPOLIA_CLASS_HASH) {
  ...
  isDeployed(provider: Pick<RpcProvider, "getClassHashAt">): Promise<boolean> {
    return isAccountDeployed(provider, this.address);
  }
