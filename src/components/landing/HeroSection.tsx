    ...
    <section id="hero" className="landing-panel landing-candy-light flex min-h-[calc(100svh-5rem)] items-center justify-center px-5 pb-24 pt-36 sm:px-8 sm:pb-28 sm:pt-40">
      ...
      <div className="relative z-10 flex w-full max-w-5xl flex-col items-center text-center">
        ...
        <form onSubmit={handleSubmit} className="landing-dock-glow mt-12 flex w-full max-w-2xl flex-col gap-2 rounded-[1.7rem] border border-white/70 bg-[#fff8f5]/70 p-2 backdrop-blur-xl sm:flex-row sm:items-center sm:rounded-full">
          <label htmlFor="hero-waitlist-entry" className="sr-only">Phone number or Starknet wallet</label>
          <button type="button" onClick={() => setInputMode(inputMode === 'phone' ? 'wallet' : 'phone')} aria-label={`Switch to ${inputMode === 'phone' ? 'wallet address' : 'phone number'} input`} className="hidden items-center gap-2 rounded-full border border-[#5d2e37]/15 bg-white/55 px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-[#6c4451] sm:flex"><span className="text-[#f15b33]">{inputMode === 'phone' ? '01' : '0x'}</span>{inputMode}</button>
          <input id="hero-waitlist-entry" type={inputMode === 'phone' ? 'tel' : 'text'} value={inputValue} onChange={(event) => setInputValue(event.target.value)} placeholder={inputMode === 'phone' ? 'Phone number' : 'Starknet wallet address'} className="min-w-0 flex-1 bg-transparent px-4 py-3 text-sm font-medium text-[#2b1621] placeholder:text-[#b88e98] focus:outline-none sm:py-2.5" />
        ...
        <p className="mt-4 font-mono text-[9px] font-bold uppercase tracking-[0.16em] text-[#805361]">Drop your number or wallet to join the early access registry.</p>
