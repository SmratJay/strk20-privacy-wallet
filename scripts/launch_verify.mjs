#!/usr/bin/env node
/**
 * ORRANGE LAUNCHPAD V2 verification — real on-chain BUY/SELL round trip + graduation
 * progress + truthful migration state.
 *
 *   node scripts/launch_verify.mjs
 *
 * Reads the live V2 curve, sells the deployer's full memecoin balance back through the real
...
async function main() {
  ...
  const curveState = async (label) => {
    ...
    console.log(`liquidity migrated   ${BigInt(mig) === 1n}`);
