...
async function main() {
  ...
  console.log('Fetching live dynamic gas bounds for PELPerpsCore declaration...');
  ...
  const bounds = {
    l2_gas: { max_amount: 550000000n, max_price_per_unit: (l2GasPrice * 18n) / 10n },
  ...
  try {
    ...
    console.log('PELPerpsCore declared successfully!');
  } catch (err) {
    console.error('PELPerpsCore declare failed:', err.message || err);
  }
}
