...
describe('REAL_GROTH16_OPEN_E2E (actual Garaga verifier)', () => {
  ...
  it('moved exactly the collateral units (margin * 10000) into the adapter', async () => {
    const bal = await provider.callContract({
      contractAddress: manifest.collateralToken,
      entrypoint: 'balance_of',
      calldata: [manifest.strk20Adapter],
    });
    const adapterBalance = uint256.uint256ToBN({ low: bal[0], high: bal[1] });
    expect(adapterBalance).toBe(marginCents * 10000n);
  });
